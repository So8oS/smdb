...
const MovieDetail = () => {
  ...
  useEffect(() => {
    axios.get(`https://api.themoviedb.org/3/movie/${movieId}?api_key=${process.env.API_KEY}`).then((res) => {
      setMovie(res.data);
    });
    axios.get(`https://api.themoviedb.org/3/movie/${movieId}/credits?api_key=${process.env.API_KEY}`).then((res) => {
      setActors(res.data.cast);
    });
    axios.get(`https://api.themoviedb.org/3/movie/${movieId}/videos?api_key=${process.env.API_KEY}`).then((res) => {
      setVideo(res.data.results[2]?.key || res.data.results[3]?.key);
    });
    axios.get(`https://api.themoviedb.org/3/movie/${movieId}/reviews?api_key=${process.env.API_KEY}`).then((res) => {
      setReviews(res.data.results);
    });
    axios
      .get(`https://api.themoviedb.org/3/movie/${movieId}/recommendations?api_key=${process.env.API_KEY}`)
      .then((res) => {
  ...
  return (
    <div className="flex flex-col">
      {movie.backdrop_path ? <img className="w-full" src={`https://image.tmdb.org/t/p/original${movie.backdrop_path}`} alt={movie.title} /> : <Skeleton highlightColor="#c9c5c5" height={400} />}
            ...
            {actors.map((actor: Actor) => {
              return (
                <Link key={actor.id} href={`/Actor/${actor.id}`} className="flex flex-col justify-center items-center min-w-fit ">
                  {actor.profile_path && <img className="w-28" src={`https://image.tmdb.org/t/p/original${actor.profile_path}`} alt={actor.name} />}
              ...
              {reviews?.map((review: Review) => {
                return (
                  <div key={review.id} className="flex flex-col border border-gray-400 p-2 gap-2 mt-3 min-w-fit shadow-2xl">
                    ...
                    <p key={review.id}>
                      {review.content.slice(0, readMore.num)}..
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (readMore.num === 200) {
                              ...
                              setReadMore({ num: 200, text: "Show More" });
                            }
                          }}
                        >
                          {readMore.text}

...
const CreditDetails = () => {
  ...
  useEffect(() => {
    axios.get(`https://api.themoviedb.org/3/person/${creditId}?api_key=${process.env.API_KEY}`).then((res) => {
      setCredit(res.data);
    });
    axios
      .get(`https://api.themoviedb.org/3/person/${creditId}/movie_credits?api_key=${process.env.API_KEY}`)
      .then((res) => {
  ...
  return (
    <div className="flex flex-col items-center mt-10 ">
      <img className="w-72 rounded-lg shadow-xl" src={`https://image.tmdb.org/t/p/original${credit.profile_path}`} alt={credit.name} />
      ...
      <div className="px-4 py-6 w-full">
        ...
        <div className="flex flex-col mb-6">
          <div className="flex items-center gap-4">
            ...
            {credit.place_of_birth && <p>{credit.place_of_birth}</p>}
            {credit.deathday && <p>Passed Away {credit.deathday}</p>}
          </div>
          <div className="mt-4">
            <h2 className="text-2xl font-bold mb-2">Biography</h2>
            <p>
              {`${bio.slice(0, readMore.num)}...`}
              <button
